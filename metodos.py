...
class MetodosCalculo:
    ...
    def generar_problema_por_metodo(self, metodo):
        ...
        if metodo == "Interpolación lineal":
            ...
            return self.generar_problema_edo(metodo)
        elif metodo in [
            ...
            return self.generar_problema_minimos_cuadrados(metodo)
...
"""
    ...
    def _resolver_edo_un_paso(self, metodo, f, x, y, h):
        """Aplica un solo paso del método de EDO indicado"""
        if metodo == "Euler modificado":
            ...
            # Runge-Kutta de 4° orden (lo usamos también para
            #  y "Runge-Kutta de orden superior")
            k1 = f(x, y)
            k2 = f(x + h / 2.0, y + (h / 2.0) * k1)
    ...
    def generar_problema_edo(self, metodo):
        """Genera un problema de EDO simple para los métodos de Euler/RK"""
        # Definimos algunas EDO sencillas
        ejemplos = [
            {
                "desc": "2x",
                "func": lambda x, y: 2 * x,
        ...
        # Escogemos un paso y número de iteraciones moderados
        ...
        y = y0
        for _ in range(n):
            y = self._resolver_edo_un_paso(metodo, f, x, y, h)
            x += h

        x_final = x0 + n * h
        respuesta = y

        problema = f"""PROBLEMA DE EDO (Método: {metodo})
...
Usa un tamaño de paso h = {h} y realiza {n} paso(s) del método de {metodo}
para aproximar el valor de y en x = {x_final:.4f}.

¿Cuál es el valor aproximado de y({x_final:.4f})?
(Responde con al menos 4 cifras decimales.)"""

        return problema, respuesta

    def generar_problema_minimos_cuadrados(self, metodo):
        """Genera un problema de ajuste por mínimos cuadrados"""
        # Definir el grado del polinomio según el método
        if "lineal" in metodo:
            grado = 1
        elif "cuadrático" in metodo:
            grado = 2
        elif "cúbico" in metodo:
            grado = 3
        else:
            grado = random.choice([1, 2, 3])

        # Generar coeficientes reales del polinomio
        coeficientes_reales = [random.uniform(-2, 2) for _ in range(grado + 1)]

        def p_real(x):
            return sum(
                coeficientes_reales[i] * (x ** (grado - i))
                for i in range(grado + 1)
            )

        # Generar puntos de datos con un poco de ruido
        xs = []
        ys = []
        for _ in range(max(4, grado + 2)):
            x = random.uniform(-3, 3)
            y = p_real(x) + random.uniform(-0.2, 0.2)
            xs.append(x)
            ys.append(y)

        # Elegir un punto donde queremos estimar usando el modelo ajustado
        x_obj = random.uniform(-2, 2)

        # Ajustar el polinomio por mínimos cuadrados
        coef_ajuste = np.polyfit(xs, ys, grado)
        # Evaluar en x_obj
        y_obj = np.polyval(coef_ajuste, x_obj)

        # Construir el texto de los puntos
        puntos_texto = "\n".join(
            [f"(x{i+1}, y{i+1}) = ({xs[i]:.3f}, {ys[i]:.3f})" for i in range(len(xs))]
        )

        problema = f"""PROBLEMA DE MÍNIMOS CUADRADOS (Método: {metodo})

Se tienen las siguientes mediciones experimentales aproximadas:

{puntos_texto}

Usando el método de {metodo}, ajusta un polinomio de grado {grado}
a los datos anteriores y estima el valor de la variable dependiente en:

    x = {x_obj:.3f}

¿Cuál es el valor aproximado de y({x_obj:.3f})?
(Responde con al menos 4 cifras decimales.)"""

        return problema, y_obj


def generar_problema_rk_simpson_tercio(self):
    problema = "Aplicar RK4 1/3 Simpson con k1,k2,k3,k4 y calcular y1."
    return problema, 0.0

def generar_problema_rk_simpson_tresoctavos(self):
    problema = "Aplicar RK4 3/8 Simpson con k1,k2,k3,k4 y calcular y1."
    return problema, 0.0
